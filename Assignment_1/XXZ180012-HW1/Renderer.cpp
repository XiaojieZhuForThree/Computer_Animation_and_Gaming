...
float rotate;
float universal_b;
float localRotate, globalRotate;
...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	gui_1->addGroup("Object Position");
	gui_1->addVariable("rotate a", rotate)->setSpinnable(true);
	gui_1->addVariable("rotate b", universal_b)->setSpinnable(true);
	gui_1->addButton("Rotate Local X", []() {
		localRotate += rotate;
		});
	gui_1->addButton("Rotate Global Y", []() {
		globalRotate += universal_b;
		});
	gui_1->addButton("Reset Object", []() {
		m_animation->reset();
		localRotate = 0;
		globalRotate = 0;
		});
...
void Renderer::load_models()
{
	...
	//added
	//main_object.obj_center = glm::vec3(0.0, 5.0, 2.0);
...
void Renderer::draw_scene(Shader& shader)
{
	...
	for (size_t i = 0; i < obj_list.size(); i++)
	{
		...
		if (obj_list[i].obj_name == "main_object")
		{
			...
			main_object_model_mat = glm::rotate(main_object_model_mat, glm::radians(localRotate), glm::vec3(1.0, 0.0, 0.0));

			glm::mat4 universal_mat = m_camera->get_view_mat();
			universal_mat = glm::rotate(universal_mat, glm::radians(globalRotate), glm::vec3(0, 1, 0));
			glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(main_object_model_mat));
			glUniformMatrix4fv(glGetUniformLocation(shader.program, "view"), 1, GL_FALSE, glm::value_ptr(universal_mat));
